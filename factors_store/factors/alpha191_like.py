...
from ..operators import (
    ...
    wide_count as Count,
    wide_covariance as Cov,
    wide_decay_linear as Decaylinear,
    wide_delay as Delay,
    wide_delta as Delta,
    wide_highday as Highday,
    wide_lowday as Lowday,
    wide_max as Max,
    wide_min as Min,
    wide_product as Prod,
    wide_rank as Rank,
    wide_regbeta as Regbeta,
    wide_returns as Returns,
    wide_rowmax as Rowmax,
    wide_rowmin as Rowmin,
    wide_sequence as Sequence,
    wide_sma as Mean,
    wide_sma_ewm as Sma,
    wide_sign as Sign,
    wide_stddev as Std,
...
class Alphas191(Alphas):
    def __init__(self, df_data):
        ...
        self.amount_df = df_data["amount"]
        self.benchmark_open_df = df_data["benchmark_open"]
        self.benchmark_close_df = df_data["benchmark_close"]
        self.returns_df = Returns(self.close_df)
        self.close_prev_df = self.close_df.shift(1)

        # 保留旧属性名，便于直接复用原始 alpha191 公式代码。
        ...
        self.close_prev = self.close_prev_df
        self.amount = self.amount_df
        self.benchmark_open = self.benchmark_open_df
        self.benchmark_close = self.benchmark_close_df
...
def _alpha_method_names() -> list[str]:
    return sorted(
        name
        for name in dir(Alphas191)
        if name.startswith("alpha") and callable(getattr(Alphas191, name))
    )


def _build_alpha191_input(data: dict[str, pd.Series]) -> pd.DataFrame:
    validate_data(data, required_fields=LIBRARY_REQUIRED_FIELDS["alpha191"])
    fields = list(LIBRARY_REQUIRED_FIELDS["alpha191"])
    for field in LIBRARY_OPTIONAL_FIELDS["alpha191"]:
        if field in data:
            fields.append(field)
    return to_worldquant_frame(data, fields=fields)


def register_alpha191(registry: FactorRegistry) -> int:
    count = 0
    for method_name in _alpha_method_names():
        factor_name = f"alpha191.{method_name}"

        def _make_factor(bound_name: str, bound_factor_name: str):
            def _factor(data: dict[str, pd.Series]) -> pd.Series:
                wide = _build_alpha191_input(data)
                engine = Alphas191(wide)
                result = getattr(engine, bound_name)()
                if isinstance(result, (int, float, np.integer, np.floating)):
                    value = float(result)
                    sample = wide["close"] * np.nan + value
                    return wide_frame_to_series(sample, name=bound_factor_name)
        ...
        registry.register(
            ...
            source="alpha191",
            required_fields=LIBRARY_REQUIRED_FIELDS["alpha191"],
...
def alpha191_source_info() -> dict[str, object]:
    return {
        "source": "alpha191",
        "path": __file__,
        "n_defs": len(_alpha_method_names()),
        "required_fields": LIBRARY_REQUIRED_FIELDS["alpha191"],
        "optional_fields": LIBRARY_OPTIONAL_FIELDS["alpha191"],
        "notes": "内部统一转成 wide DataFrame，在类内按原始 Alpha191 风格维护公式逻辑。",
    }
